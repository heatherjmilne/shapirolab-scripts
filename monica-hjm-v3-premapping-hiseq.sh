...
bash ${GET_SAMPLES} ${PREFIX} l > ${PROCESSING_OUTPUT}/${PREFIX}-sample-list-${DATE}.txt
...
#Unzips original file, then makes a symlink of the unzipped file. Uses the existing symlink (Generated by Nathan's script for easier navigation of the Fulgent file system
for SAMPLE in $(cat ${PREFIX}-sample-list-${DATE}.txt)
do
	gzip -d $(readlink -f ${RAW_READ_DIRECTORY}/${SAMPLE}*.fastq.gz)
	wait
	ORIG_R1=$(readlink -f ${RAW_READ_DIRECTORY}/${SAMPLE}_L001_R1_001.fastq.gz)
	ORIG_R2=$(readlink -f ${RAW_READ_DIRECTORY}/${SAMPLE}_L001_R2_001.fastq.gz)
	echo $ORIG_R1
	echo $ORIG_R2
	ln -s ${ORIG_R1%.*} ${PROCESSING_OUTPUT}/Raw_data_symlinks/${SAMPLE}_L001_R1_001.fastq
	wait
	ln -s ${ORIG_R2%.*} ${PROCESSING_OUTPUT}/Raw_data_symlinks/${SAMPLE}_L001_R2_001.fastq
	wait
done
wait
